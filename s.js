...
const getInstruction = (recept, r = 0) => {
    if (Array.isArray(recept)) {
        for (let o of recept) getInstruction(o, r++)
        return
    }

    console.log(r + " добавьте " + TYPES.reagent.Reagents[recept.name].name + " " + recept.col + " ЕД. ")
}
getInstruction(getRecept("Cognizine", 9))
console.log((getRecept("Cognizine", 9)))

for (let a of Object.keys(PrototypeReader.TYPES.reaction.Reactions)) {
    let b = PrototypeReader.TYPES.reagent.Reagents[a]
    if (b)
